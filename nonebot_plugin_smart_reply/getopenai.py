...
class Openai:
    ...
    async def get_usage(
        ...
        try:
            async with AsyncClient() as client:
                usage_result = (await client.get(self.host + self.usage_url, headers=headers, params=params)).json()
                subscription_result = (await client.get(self.host + self.subscription_url, headers=headers)).json()
            # 获取总额
    ...
    async def apikey_status(
        ...
        if not self.apikey_allow:
            ...
            tasks = [self.get_usage(params=params, apikey=apikey) for apikey in utils.openai_api_key]
