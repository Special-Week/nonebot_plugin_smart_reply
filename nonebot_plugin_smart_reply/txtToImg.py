...
def txt_to_img(text, font_size=30, font_path="simsun.ttc") -> bytes:
    text = line_break(text)
    d_font = ImageFont.truetype(font_path, font_size)
    lines = text.count('\n')  # 计算行数
    image = Image.new("L", (LINE_CHAR_COUNT*font_size // 2+ 50, font_size*lines+50), "white")
    draw_table = ImageDraw.Draw(im=image)
    draw_table.text(xy=(25, 25), text=text, fill='#000000', font= d_font, spacing=4)  # spacing调节机制不清楚如何计算
